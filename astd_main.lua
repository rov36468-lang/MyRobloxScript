...
-- =========================================================================
local macroData = {}        -- เก็บข้อมูล Action ที่บันทึก
local isRecording = false
local isPlaying = false
local isLooping = false
local recordConn = nil
local lastActionTime = 0
local statusLabel = nil     -- Label แสดงสถานะ

local UIS = game:GetService("UserInputService")
local mouse = player:GetMouse()

-- ฟังก์ชัน simulate คลิก (รองรับหลาย Executor)
...
local function startRecording()
    ...
    macroData = {}
    lastActionTime = tick()
    updateStatus("🔴 กำลังบันทึก... (คลิก/กดคีย์)", Color3.fromRGB(255, 80, 80))

    recordConn = UIS.InputBegan:Connect(function(input, gp)
        ...
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            table.insert(macroData, {
                type = "click",
                x = mouse.X, y = mouse.Y,
                wx = mouse.Hit.X, wy = mouse.Hit.Y, wz = mouse.Hit.Z,
                delay = delay
            })
            updateStatus("🔴 บันทึก: " .. #macroData .. " actions", Color3.fromRGB(255, 80, 80))
...
local function stopRecording()
    ...
    updateStatus("✅ บันทึกเสร็จ: " .. #macroData .. " actions", Color3.fromRGB(80, 255, 150))
end

local function runMacroOnce()
    for _, action in ipairs(macroData) do
        if not isPlaying then break end
        task.wait(math.max(action.delay, 0.05))
        if action.type == "click" then
            simulateClick(action.x, action.y)
        elseif action.type == "key" then
            simulateKey(action.keyCode)
        end
    end
end

local function playMacro()
    if isPlaying or isRecording or #macroData == 0 then
        if #macroData == 0 then
            updateStatus("⚠️ ยังไม่มีมาโครที่บันทึก!", Color3.fromRGB(255, 200, 50))
        end
        return
    end
    isPlaying = true
    isLooping = false
    updateStatus("▶️ กำลังเล่นมาโคร...", Color3.fromRGB(100, 220, 255))

    task.spawn(function()
        runMacroOnce()
        isPlaying = false
        updateStatus("✅ เล่นมาโครเสร็จสิ้น!", Color3.fromRGB(80, 255, 150))
    end)
end

local function loopMacro()
    if isPlaying or isRecording or #macroData == 0 then
        if #macroData == 0 then
            updateStatus("⚠️ ยังไม่มีมาโครที่บันทึก!", Color3.fromRGB(255, 200, 50))
        end
    ...
    isPlaying = true
    isLooping = true
    updateStatus("🔁 Loop มาโคร กำลังทำงาน...", Color3.fromRGB(100, 220, 255))
    ...
    task.spawn(function()
        ...
        isPlaying = false
        updateStatus("⏹️ หยุด Loop แล้ว", Color3.fromRGB(180, 225, 255))
    end)
end
...
local function stopMacro()
    ...
    updateStatus("⏹️ หยุดทำงานแล้ว", Color3.fromRGB(180, 225, 255))
end

-- ฟังก์ชันสร้างปุ่มกดในหน้าเพจ
local function createActionButton(pageFrame, text, bgColor, layoutOrder, callback)
    ...
    btn.MouseEnter:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(
            math.min(bgColor.R*255+30, 255),
            math.min(bgColor.G*255+30, 255),
            math.min(bgColor.B*255+30, 255)
        )}):Play()
    end)
...
-- =========================================================================
-- หน้ามาโคร — ปุ่มและ UI จริง
-- =========================================================================
createSubHeader(pageMacro, "ระบบบันทึกมาโคร", 1)

-- Status Label
local macroStatus = Instance.new("TextLabel")
macroStatus.Size = UDim2.new(1, 0, 0, 30)
macroStatus.BackgroundColor3 = Color3.fromRGB(6, 16, 38)
macroStatus.Text = "  ⏸ พร้อมใช้งาน — กด 'เริ่มบันทึก' ก่อน"
macroStatus.TextColor3 = Color3.fromRGB(180, 225, 255)
macroStatus.TextSize = 12
macroStatus.Font = Enum.Font.GothamMedium
macroStatus.TextXAlignment = Enum.TextXAlignment.Left
macroStatus.LayoutOrder = 2
macroStatus.Parent = pageMacro
Instance.new("UICorner", macroStatus).CornerRadius = UDim.new(0, 6)
local sp = Instance.new("UIPadding", macroStatus); sp.PaddingLeft = UDim.new(0, 8)
statusLabel = macroStatus

createActionButton(pageMacro, "🔴  เริ่มบันทึก (Record)", Color3.fromRGB(180, 30, 30), 3, startRecording)
createActionButton(pageMacro, "⏹️  หยุดบันทึก (Stop)", Color3.fromRGB(60, 60, 100), 4, stopRecording)

createSubHeader(pageMacro, "เล่นมาโคร", 5)
createActionButton(pageMacro, "▶️  เล่นครั้งเดียว (Play Once)", Color3.fromRGB(20, 100, 60), 6, playMacro)
createActionButton(pageMacro, "🔁  เล่นวนซ้ำ (Loop)", Color3.fromRGB(20, 70, 160), 7, loopMacro)
createActionButton(pageMacro, "⏹️  หยุดทุกอย่าง (Stop All)", Color3.fromRGB(80, 30, 120), 8, stopMacro)

createSubHeader(pageMacro, "ล้างมาโคร", 9)
createActionButton(pageMacro, "🗑️  ล้างมาโครทั้งหมด (Clear)", Color3.fromRGB(100, 40, 10), 10, function()
    macroData = {}
    stopMacro()
    updateStatus("🗑️ ล้างมาโครเรียบร้อย", Color3.fromRGB(255, 200, 50))
end)
...
createListItem(pageHome, "เวอร์ชันสคริปต์: พี่หล่อไหมน้อง Hub v1.1.0", 5)
createListItem(pageHome, "ระบบมาโคร: พร้อมใช้งาน ✅", 6)
